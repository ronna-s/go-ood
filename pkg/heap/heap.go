...
// Heap is a slice with heap properties of any type that satisfies the Ordered constraint
type Heap[T interface{ Less(T) bool }] []T

// New returns a new heap of type Ordered
func New[T interface{ Less(T) bool }](s []T) *Heap[T] {
	h := Heap[T](s)
	h.Init()
	return &h
}
